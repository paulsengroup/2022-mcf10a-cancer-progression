...
def overlap_bedpe(df1: pd.DataFrame, df2: pd.DataFrame) -> pd.DataFrame:
    # Overlap first triplet
    df3 = bf.overlap(df1, df2, cols1=("chrom1", "start1", "end1"), cols2=("chrom1", "start1", "end1")).dropna()
    df = df1.copy().set_index(["chrom1", "start1", "end1"]).sort_index()
    df["has_overlap"] = True
    ...
    for key, grp in df3.groupby(["chrom1", "start1", "end1"]):
        ...
        overlaps = bf.overlap(
            df4, df5, cols1=("chrom2", "start2", "end2"), cols2=("chrom2_", "start2_", "end2_")
        ).dropna()
        if len(overlaps) == 0:
            df.loc[key, "has_overlap"] = False
